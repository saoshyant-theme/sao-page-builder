...
add_filter('sao_element_options', 'sao_post_featured_options');
function sao_post_featured_options( $element ) {
	$option = array();
	
 	$item = array(
 		'name'			=> 	esc_html('Post Featured','sao'),
 		'id'			=> 'post_featured',
		'img'			=>  plugin_dir_url( __DIR__ ).'assets/image/featured_grid.jpg'
  	); 
...
add_filter('sao_builder_perview_post_featured', 'sao_builder_perview_post_featured');
function sao_builder_perview_post_featured( $args ) {
	...
	$key = $args['key'];
		...
		$output ='<img src="'.plugin_dir_url( __DIR__ ).'assets/image/featured_grid.jpg">'; 
		$css.= '.sao-element-'.$key.' {text-align: center;}'; 
  	$return['css']= $css;
	$return['output']= $output;
	return $return;
}
add_filter('sao_builder_post_featured', 'sao_post_featured_config');
function sao_post_featured_config( $args ) {
	...
	$responsive = 'sao_post_column_1_'.$column_main.' sao_post_desktop_1_'.$column_desktop.' sao_post_tablet_1_'.$column_tablet.' sao_post_phablet_1_'.$column_phablet.'  sao_post_phone_1_'.$column_phone;
 	...
 	$output.='<aside class="'.esc_attr($ratio).' sao-post-featured-warp '.esc_attr($responsive) .'">'; 
  		$output.= sao_title_box($option);	
			$output.='<div class="sao-post-list"  >';
				$output.= sao_featured_post(1,$option);
			$output.='</div>';
 			$output.= sao_load_more($option,'sao_featured_post');		
  		$output.= '</aside>';			
...
function sao_featured_post($none_ajax=false,$option =false,$slider = false) { 
   	$arge = array(
		'thumb'				=>'full',
...
add_action('wp_ajax_nopriv_sao_featured_post', 'sao_featured_post');
add_action('wp_ajax_sao_featured_post', 'sao_featured_post'); 
