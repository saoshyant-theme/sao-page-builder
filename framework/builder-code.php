		...
		$rand = rand(100000, 999999);
 		...
 		$output.='[sao_'.$rand.' attr_key="'.sanitize_title($key).'"]';
			...
			if(is_array($value)){
				foreach($value as $key_2 => $value_2) :
					...
					$rand_2 = rand(100000, 999999);
						...
						if(is_array($value_2)){
									...
									if(is_array($value_3)){
										...
										if(!empty($value_4)):
												...
												$output.='[sao_'.$rand.'_'.$rand_2.'_'.$rand_3.'_'.$rand_4.' attr_key="'.sanitize_title($key_4).'"]';
  												$output.= urlencode(stripslashes($value_4)) ;
												$output.='[/sao_'.$rand.'_'.$rand_2.'_'.$rand_3.'_'.$rand_4.']';
												
										endif;
  										endforeach;
									}else{
										$output.=urlencode(stripslashes($value_3));
									}									
							...
							$output.=urlencode(stripslashes($value_2));
						}
				...
				$output.= urlencode(stripslashes($value));
			}
   					...
   					foreach($option_3 as $value_3) :
						...
						if(is_array($option_4)){
							foreach($option_4 as $value_4) :
								$array[stripslashes($value['key'])][stripslashes($value_2['key'])][stripslashes($value_3['key'])][stripslashes($value_4['key'])] = urldecode(stripslashes($value_4['value']));
							endforeach;
						}else{
							$array[stripslashes($value['key'])][stripslashes($value_2['key'])][stripslashes($value_3['key'])] = urldecode(stripslashes($value_3['value']));
						}
						
 					endforeach;
				}else{
					$array[stripslashes($value['key'])][stripslashes($value_2['key'])] = urldecode(stripslashes($value_2['value']));
				}
			endforeach;
		
		
 	}else{
	 $array[stripslashes($value['key'])] = urldecode(stripslashes($value['value']));
	}
